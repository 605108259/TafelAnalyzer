...
def export_processed_txt(path: Path, prepared: PreparedSeries, fit: TafelFit) -> Path:
...
def export_fit_npz(path: Path, prepared: PreparedSeries, fit: TafelFit) -> Path:
    np.savez(
        path,
...
def plot_tafel(out_png: Path, prepared: PreparedSeries, fit: TafelFit) -> Path:
    ...
    if fit_indices.size:
        ax0.scatter(prepared.e[fit_indices], prepared.j[fit_indices], s=24, label="拟合点")
    ax0.set_xlabel(prepared.e_label)
    ax0.set_ylabel(prepared.j_label)
    ...
    ax1.set_ylabel(prepared.tafel_y_label)
